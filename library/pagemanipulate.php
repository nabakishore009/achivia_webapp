...
$accesscontent = $accesscontent . "RewriteRule ^{$home_pgurll}/?$ contact.php [NC,L]\n";
...
$db->Fetch(TABLE_SERVICES,NULL,NULL);
    ...
    foreach($db->Data as $v)
    {
            $url=Static_Operation :: removeSpchar($v['page_url']);
            if($url){
            $accesscontent = $accesscontent . "RewriteRule ^"test-preparation/.$url."/?$ test_details.php.php?id=".$v['id']."  [QSA]\n";
            }
    }

...
if(isset($_POST) && !empty($_POST)){   
    ...
    if(isset($_POST['id'])){ 
                            ...
                            {
                                $db->FetchSingle(TABLE_GALLERY,'image',array(array('where','id',$_POST['id'])));
                            ...
                            { 
                                    if($db->Update(TABLE_GALLERY,array('image'=>$url,'tag'=>$_POST['tag']),array(array('where','id',$_POST['id']))))
                            {
                                $_SESSION['success_msg']="Gallery Successfully Updated";
                            }else{
                                $_SESSION['error_msg']="Gallery Update failed";
                            }
                ...
                if($_FILES['banner']['name']!="")
                {
                            ...
                            if($db->Insert(TABLE_GALLERY,array('image'=>$url,'tag'=>$_POST['tag'],'status'=>1)))
                            {
                                $_SESSION['success_msg']="Image Successfully Inserted";
                            }else{
                                $_SESSION['error_msg']="Image Insert failed";
                            }
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                       <h4><button class="btn btn-success" data-toggle="modal" data-target="#myModal">Add Image</button></h4>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Delete</th>
                                                                        ...
                                                                        <tbody>
<?php $xx=1;
$db->Fetch(TABLE_GALLERY,NULL,NULL," order by id DESC");
...
?>                                    
                                        <tr class="odd gradeX" id="row<?= $xx;?>">
                                            ...
                                            <div class="onoffswitch">
                                                <input type="checkbox" onClick="status('<?=TABLE_GALLERY;?>','status','<?= $v['id']?>')"  name="onoffswitch" class="onoffswitch-checkbox" id="myonoffswitch<?= $v['id']?>" <?php if($v['status']==1){ echo "checked";} ?> >
                                            ...
                                            <td align="center"class="center"><a href="javascript:" onClick="delete_col('<?=TABLE_GALLERY;?>','row<?= $xx;?>','<?= $v['id']?>')"><i class="fa fa-trash-o fa-2x"></i></a></td>
                                        </tr>
<?php $xx++;}?>                                        
                                    </tbody>
